...
void WriteSystemInfo()
{
	...
	else if (ret=='2')
	{
		...
		free(temp);
		//sysInfo->MACAddr=getul(sysInfo->MACAddr,16);
		EraseBlock(block);
		memcpy(GlobalSysBuffer,buffer,512);
		WritePage(address,buffer);
	}
	else if (ret=='3')
	{
		...
		sysInfo->LinuxKernelImageStart=getul(sysInfo->LinuxKernelImageStart,16);
		EraseBlock(block);
		memcpy(GlobalSysBuffer,buffer,512);
		WritePage(address,buffer);
	}
	else if (ret=='4')
	{
		...
		sysInfo->LinuxKernelImageSize=getul(sysInfo->LinuxKernelImageSize,16);
		EraseBlock(block);
		memcpy(GlobalSysBuffer,buffer,512);
		WritePage(address,buffer);
	}
	else if (ret=='5')
	{
		...
		sysInfo->LinuxFileSystemStart=getul(sysInfo->LinuxFileSystemStart,16);
		EraseBlock(block);
		memcpy(GlobalSysBuffer,buffer,512);
		WritePage(address,buffer);
	}
	else if (ret=='6')
	{
		...
		sysInfo->LinuxFileSystemSize=getul(sysInfo->LinuxFileSystemSize,16);
		EraseBlock(block);
		memcpy(GlobalSysBuffer,buffer,512);
		WritePage(address,buffer);
	}
	else if (ret=='7')
	{
		...
		sysInfo->OtherKernelImageStart=getul(sysInfo->OtherKernelImageStart,16);
		EraseBlock(block);
		memcpy(GlobalSysBuffer,buffer,512);
		WritePage(address,buffer);
	}
	else if (ret=='8')
	{
		...
		sysInfo->OtherKernelImageSize=getul(sysInfo->OtherKernelImageSize,16);
		EraseBlock(block);
		memcpy(GlobalSysBuffer,buffer,512);
		WritePage(address,buffer);
	}
	else if (ret=='9')
	{
		...
		sysInfo->ApplicationLoadAddress=getul(sysInfo->ApplicationLoadAddress,16);
		EraseBlock(block);
		memcpy(GlobalSysBuffer,buffer,512);
		WritePage(address,buffer);
	}
...
void ShowSystemInfo()
{
	...
	putstr("IP Address:");
	putstr(CS8900IP);
	printf("\n\rMAC Address:0x%02lx:0x%02lx:0x%02lx:0x%02lx:0x%02lx:0x%02lx\n\r",sysInfo->MACAddr[0],sysInfo->MACAddr[1],sysInfo->MACAddr[2],sysInfo->MACAddr[3],sysInfo->MACAddr[4],sysInfo->MACAddr[5]);
	printf("Boot Image at NAND Flash:0x%08lx\n\r",sysInfo->bootImageStart);
	printf("Boot Image size:0x%08lx\n\r",sysInfo->bootImageSize);
	printf("Linux Kernel Image at NAND Flash:0x%08lx\n\r",sysInfo->LinuxKernelImageStart);
	printf("Linux Kernel Image size:0x%08lx\n\r",sysInfo->LinuxKernelImageSize);
	printf("Linux File System at NAND Flash:0x%08lx\n\r",sysInfo->LinuxFileSystemStart);
	printf("Linux File System size:0x%08lx\n\r",sysInfo->LinuxFileSystemSize);
	printf("Other Kernel at NAND Flash:0x%08lx\n\r",sysInfo->OtherKernelImageStart);
	printf("Other Kernel Image size:0x%08lx\n\r",sysInfo->OtherKernelImageSize);
	printf("Application Load address in ram:0x%08lx\n\r",sysInfo->ApplicationLoadAddress);
	//printf("Linux Boot Parameters:%s\n\r",sysInfo->BootParam);
	putstr("Linux Boot Parameters:");
	putstr(sysInfo->BootParam);
	printf("\n\rBoot:Default to startup ");
...
void PerformFdisk()
{
	...
	if (ret=='1')
	{
		...
		//scanf("0x%08lx",&address);
		address=getul(address,16);
...
extern void _vector(void);

int main(void)
{
 	int ret;
 	u_int MyMMUBase,i=0;
 	char *VectDest=(char *)0x0;

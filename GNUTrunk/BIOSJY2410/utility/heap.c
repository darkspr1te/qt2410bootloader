 ...
 */
#include "sys.h"
#define HEAP_BASE 0x335f8000
#define HEAP_SIZE SIZE_2M
...
static inline int malloc_init(unsigned char *heap, unsigned long size)
{
	if (gHeapBase != NULL) return -1;

	//printf("malloc_init(): initialize heap area at 0x%08lx, size = 0x%08lx\n\r",heap, size);
...
int heap_init(void)
{
	return malloc_init((unsigned char *)(HEAP_BASE), HEAP_SIZE);	
}
